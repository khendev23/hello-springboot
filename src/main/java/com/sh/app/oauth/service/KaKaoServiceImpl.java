...
public class KaKaoServiceImpl implements KaKaoService {
	 ...
	 * token
	 * - 한정된 권한정보, 만료기한 있는 키
	 * - access_token : 사용자 정보, 특정권한 수행할 수 있는 키, 만료시간이 짧음(보통 30초)
	 * - refresh_token : access_token을 재발급 용도의 키. 만료시간이 길다.(보통 1~2시간)
	 * - 예전에는 db등에 token을 보관 후 꺼내썼지만, 최근엔 session 보관을 추천한다.
	 */
	@Override
	public Map<String, Object> getTokens(String code) {
		RestTemplate restTemplate = new RestTemplate(); // 타 서버로의 요청객체
		// 요청 header, 사용자입력값
		...
		String uri = TOKEN_URI 
					...
					+ "&redirect_uri=" + REDIRECT_URI
					+ "&code=" + code;
		ResponseEntity<?> responseEntity = restTemplate.exchange(URI.create(uri), HttpMethod.GET, httpEntity, Map.class);
	...
	public Map<String, Object> getProfile(String accessToken) {
		...
		// 헤더에 Authorization : Bearer  XXXXXX  액세스 토큰 설정
		HttpHeaders httpHeaders = new HttpHeaders();
		httpHeaders.setBearerAuth(accessToken);
		HttpEntity<HttpHeaders> httpEntity = new HttpEntity<>(httpHeaders);
		String uri = RESOURCE_URI_HOST + "/v2/user/me";
		ResponseEntity<Map<String, Object>> responseEntity = 
				restTemplate.exchange(URI.create(uri), HttpMethod.GET, httpEntity, new ParameterizedTypeReference<Map<String, Object>>(){});
